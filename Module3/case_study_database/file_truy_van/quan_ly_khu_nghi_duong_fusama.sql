...
h.ngay_lam_hop_dong, h.ngay_ket_thuc,  (ifnull(d.chi_phi_thue,0) + ifnull(t.so_luong,0) * ifnull(dk.gia,0)) tong_tien from khach_hang k
...
kh.so_dien_thoai as so_dien_thoai_khach_hang, dv.ten_dich_vu, 
...
having max(ct.so_luong) >= all (select max(ct.so_luong) from hop_dong_chi_tiet ct group by ct.ma_dich_vu_di_kem);
